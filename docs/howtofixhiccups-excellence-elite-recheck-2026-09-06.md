...
Date: 2026-09-07 00:55 UTC (updated after `5aee021` restored Privacy 58s after the last drop)  
...
PR head at check: `5aee021384bc8bdb3c9dcbb7f35759c74fc4f62f` (draft)  
...
- **One-line why:** Twenty-fifth flip is a restore that lasted 58 seconds after the drop. Header Privacy is live on this GET. Hero is still the FORCE remint. Do not call YES.
...
| Copy | NO | Twenty-fifth restore. Header Privacy live on `/` `/why/` `/when/` `/privacy/` and 404. Git matches served. `preview-html.js` deleted. CI flipped back to four links. HUMAN.md nav line is four links. Top banner still says three-link Mike lock. Plate still H1+Start. Chrome is not held. |
...
Prior wiring elite YES was overridden for step fidelity. The 22:10 pass called YES on live files. Header chrome then flipped forty-four times. Edge strip is gone on this GET. Hero is a 2.5s plastic-straw FORCE remint at 0.45x with H1+Start only. Step clips 01-04 still MATCH. Wired hero and plate do not.
