...
Date: 2026-09-07 00:59 UTC (updated after `f00b6c9` dropped Privacy 76s after the last restore)  
...
PR head at check: `f00b6c9e7180129bccb24671f43321c3e85135a0` (draft)  
...
- **One-line why:** Privacy dropped again 76 seconds after the last restore. Edge strip is back and public. Hero is still the FORCE remint.
...
| Copy | NO | Twenty-sixth drop. Header Privacy gone on `/` `/why/` `/when/` `/privacy/` and 404. Git matches served (three links). `preview-html.js` is back and GET 200. `netlify.toml` registers it on `/*`. CI flipped back to three links. HUMAN.md nav line is three links again. Plate still H1+Start. |
...
Prior wiring elite YES was overridden for step fidelity. The 22:10 pass called YES on live files. Header chrome then flipped forty-seven times. Edge strip is back on this GET. Hero is a 2.5s plastic-straw FORCE remint at 0.45x with H1+Start only. Step clips 01-04 still MATCH. Wired hero and plate do not.
