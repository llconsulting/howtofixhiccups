...
Date: 2026-09-07 00:27 UTC (updated after `e010a5f` restored header Privacy and flipped the nav CI check)  
...
PR head at check: `e010a5fe38ffcb72a5368ccd5a1dfe688a27aeff` (draft)  
...
- **One-line why:** Privacy is back and the CI check now requires four header links. Hero is still the FORCE plastic-straw remint. Do not call YES. The two nav jobs are now fighting with build scripts.
...
| Copy | NO | Header chrome matches again after `e010a5f`. HUMAN.md four-link text is restored. Plate is still H1+Start only. Header YES is not a Copy YES. The public nav script is still 200. |
...
| HUMAN excellence copy | FAIL | Body paste still on the page. Four-item header is back. Plate is still H1+Start only. Competing CI locks are not a held chrome decision. |
...
- `/scripts/check-primary-nav.sh` still GET 200. Script now requires four header links. HUMAN deck md still 404.
...
Prior wiring elite YES was overridden for step fidelity. The 22:10 pass called YES on live files. Header chrome then flipped thirty-four times. Nav CI now requires four links. Hero is a 2.5s plastic-straw FORCE remint at 0.45x with H1+Start only. Step clips 01-04 still MATCH. Wired hero and plate do not.
