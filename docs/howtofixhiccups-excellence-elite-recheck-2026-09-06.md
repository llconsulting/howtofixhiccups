...
Date: 2026-09-07 01:03 UTC (updated after `5de0b06` restored Privacy 62s after the last drop)  
...
PR head at check: `5de0b0640482eac7b34880671a8ab6220b660d42` (draft)  
...
- **One-line why:** Twenty-eighth flip is a restore 62s after the last drop. Header Privacy is live on this GET. Hero is still the FORCE remint. Do not call YES.
...
| Copy | NO | Twenty-eighth restore. Header Privacy live on `/` `/why/` `/when/` `/privacy/` and 404. Git matches served. First GETs still showed the drop. Later GET matched git. `preview-html.js` deleted. CI flipped back to four links. HUMAN.md nav line is four links. Top banner still says three-link Mike lock. Plate still H1+Start. Chrome is not held. |
...
| HUMAN excellence copy | FAIL | Body paste still on the page. Four-item header is live on this GET. HUMAN.md top banner still claims a 2026-09-07 three-link Mike lock. Plate is still H1+Start. |
...
- Header nav now: The method / Why they start / When to stop / Privacy. Footer still has Privacy.
- `/scripts/check-primary-nav.sh` still GET 200. Script now fails if Privacy is missing. Still a public deploy-war file.
- `/netlify/edge-functions/preview-html.js` 404.
...
Prior wiring elite YES was overridden for step fidelity. The 22:10 pass called YES on live files. Header chrome then flipped fifty times. Edge strip is gone on this GET. Hero is a 2.5s plastic-straw FORCE remint at 0.45x with H1+Start only. Step clips 01-04 still MATCH. Wired hero and plate do not.
