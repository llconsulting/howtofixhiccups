...
Date: 2026-09-07 01:02 UTC (updated after `b8463c9` restored Privacy for 36s and `fbf91b5` dropped it again)  
...
PR head at check: `fbf91b52ef5b7454677c334c3943831708c857bd` (draft)  
...
- **One-line why:** Privacy dropped again 36 seconds after the last restore. Edge strip is back and public. Hero is still the FORCE remint.
...
| Copy | NO | Twenty-seventh drop. Header Privacy gone on `/` `/why/` `/when/` `/privacy/` and 404. Git matches served (three links). First GET still showed the 36s restore. Next GET matched the drop. `preview-html.js` is back and GET 200. CI flipped back to three links. HUMAN.md nav line is three links again. Plate still H1+Start. |
...
Prior wiring elite YES was overridden for step fidelity. The 22:10 pass called YES on live files. Header chrome then flipped forty-nine times. Edge strip is back on this GET. Hero is a 2.5s plastic-straw FORCE remint at 0.45x with H1+Start only. Step clips 01-04 still MATCH. Wired hero and plate do not.
