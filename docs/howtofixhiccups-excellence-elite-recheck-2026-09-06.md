...
Date: 2026-09-06 23:28 UTC (updated after `37b1e77` dropped header Privacy a twelfth time)  
...
PR head at check: `37b1e77937be9b1e0a95c882957ae1ad1e8bf00d` (draft)  
...
- **One-line why:** Header Privacy dropped a twelfth time, 30s after the eleventh restore; hero is still a 0.45x sip with H1+Start only. Step clips still MATCH.
...
| Copy | NO | Twelfth Privacy drop on `/` `/why/` `/when/` `/privacy/` and 404. Footer still has Privacy. Hero plate is still H1+Start only. HUMAN stack still in `band-intro`. |
...
| HUMAN excellence copy | FAIL | Body paste still on the page. Locked four-item header is gone a twelfth time. Hero plate is still H1+Start only. |
...
- Header nav now: The method / Why they start / When to stop. Privacy gone a twelfth time.
...
Prior wiring elite YES was overridden for step fidelity. The 22:10 pass called YES on live files. Header chrome then flipped twenty-three times. Hero is a full-opacity 0.45x sip with H1+Start only. Step clips 01-04 still MATCH. Wired hero, plate, and chrome do not.
