...
Date: 2026-09-07 00:58 UTC (updated after `a32eb71` restored Privacy 68s after the last drop)  
...
PR head at check: `a32eb714b05a8b7e817206d1d0c28614c19f2de6` (draft)  
...
- **One-line why:** Twenty-sixth flip is a restore 68s after the last drop. Header Privacy is live on this GET. Hero is still the FORCE remint. Do not call YES.
...
| Copy | NO | Twenty-sixth restore. Header Privacy live on `/` `/why/` `/when/` `/privacy/` and 404. Git matches served. `preview-html.js` deleted. CI flipped back to four links. HUMAN.md nav line is four links. Top banner still says three-link Mike lock. Plate still H1+Start. Chrome is not held. |
...
Prior wiring elite YES was overridden for step fidelity. The 22:10 pass called YES on live files. Header chrome then flipped forty-six times. Edge strip is gone on this GET. Hero is a 2.5s plastic-straw FORCE remint at 0.45x with H1+Start only. Step clips 01-04 still MATCH. Wired hero and plate do not.
