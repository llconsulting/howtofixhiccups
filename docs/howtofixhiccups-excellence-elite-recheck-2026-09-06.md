...
Date: 2026-09-07 00:53 UTC (updated after `3c5eecc` restored Privacy and deleted preview-html again)  
...
PR head at check: `3c5eeccd3be3142cac4972668617ac80695e4391` (draft)  
...
- **One-line why:** Twenty-fourth flip is a restore. Header Privacy is live on this GET. Edge strip is gone. Hero is still the FORCE remint. Do not call YES on a restore.
...
| Copy | NO | Twenty-fourth restore. Header Privacy live on `/` `/why/` `/when/` `/privacy/` and 404. Git matches served. `preview-html.js` deleted. CI flipped back to four links. HUMAN.md nav line is four links. Top banner still says three-link Mike lock. Plate still H1+Start. Chrome is not held. |
...
Prior wiring elite YES was overridden for step fidelity. The 22:10 pass called YES on live files. Header chrome then flipped forty-two times. Edge strip is gone on this GET. Hero is a 2.5s plastic-straw FORCE remint at 0.45x with H1+Start only. Step clips 01-04 still MATCH. Wired hero and plate do not.
