...
Date: 2026-09-06 22:30 UTC (updated after `e3f3873` dropped header Privacy again)  
...
PR head at check: `e3f38733c873d149481f3e05863fa3bde81998db` (draft)  
Restore head: `fbce82af214b7e263e9a7d0ff9bd006123839c47`  
...
- **One-line why:** Clips and hero still MATCH; `e3f3873` dropped locked header Privacy again after `fbce82a` restored it.
...
| Copy | NO | Body strings still match. Locked header chrome does not. Live header on `/` `/why/` `/when/` `/privacy/` and 404 is The method / Why they start / When to stop only. Footer still has Privacy. This is the second drop (`e3f3873` after `01aee06`). Footer-only is not a match. |
...
| HUMAN excellence copy | FAIL | Body paste still matches. Locked four-item header is gone again after `e3f3873`. Footer-only Privacy is not a match. |
...
- Header nav now: The method / Why they start / When to stop. Privacy gone again.
...
Prior wiring elite YES was overridden for step fidelity. The 22:10 pass called YES on live files. `01aee06` broke locked header chrome. `fbce82a` restored it. `e3f3873` broke it again.
