...
if [[ -f netlify/edge-functions/preview-html.js ]]; then
  echo "FAIL: preview-html edge function is present and strips header Privacy"
  fail=1
fi
...
if grep -q 'function = "preview-html"' netlify.toml; then
  echo "FAIL: netlify.toml still registers preview-html"
  fail=1
fi
